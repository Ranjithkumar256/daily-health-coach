/**
 * Daily Health Coach - Google AdMob Monetization Manager
 * Renders high-converting responsive AdMob Banner & Interstitial Ads
 * to monetize your app on Google Play Store.
 */
  ...
  const AdsManager = {
    ...
    injectBannerStyles() {
      ...
      style.textContent = `
        ...
        /* Pad body so bottom banner doesn't cover content */
        ...
        @media (max-width: 875px) {
          ...
          body.has-admob-banner .app-content,
          body.has-admob-banner .main-content {
      ...
      banner.innerHTML = `
        <div class="admob-banner-content">
          ...
          <div class="admob-text">
            <strong>Optimal Electrolytes & Whey</strong><br>
            Zero Sugar hydration formulas for all-day energy.
          </div>
          <button class="admob-cta" onclick="window.AdsManager.onAdClick()">Learn More</button>
        </div>
      `;
    ...
    onAdClick() {
      // Directs to safe health sponsor or Play Store ad partner
    ...
    showInterstitial(message = 'Great Progress!') {
      ...
      if (!navigator.onLine) return; // Ads only load when online

      this.interstitialCooldown = true;
      setTimeout(() => { this.interstitialCooldown = false; }, 60000); // 1 min cooldown
      ...
      overlay.innerHTML = `
        <div class="admob-interstitial-card">
          ...
          <p style="color: #94a3b8; font-size: 0.82rem; margin-bottom: 20px;">
            Support Daily Health Coach free access by checking out our featured wellness sponsors!
          </p>
          <div style="background: rgba(255,255,255,0.04); border: 1px dashed rgba(255,255,255,0.15); border-radius: 10px; padding: 14px; margin-bottom: 16px;">
            <div style="font-weight: 700; color: #38bdf8; font-size: 0.9rem;">Clean Energy Multi-Electrolytes</div>
            <div style="font-size: 0.78rem; color: #cbd5e1; margin-top: 4px;">Zero artificial colors • 100% natural stevia • Fast rehydration</div>
          </div>
      ...
      const interval = setInterval(() => {
        ...
        if (secondsLeft > 0) {
          ...
          closeBtn.textContent = 'Continue to Health Coach ✓';
