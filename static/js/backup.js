...
(function(window) {
  ...
  const BackupManager = {
    ...
    isTermsAccepted() {
      try {
        const sessionAccepted = sessionStorage.getItem('dhc_terms_accepted_session');
    ...
    updateProfileTermsBadge() {
      const badge = document.getElementById('profileTermsStatusBadge');
      if (badge) {
        const accepted = this.isTermsAccepted() || localStorage.getItem(TERMS_CONSENT_KEY);
        if (accepted) {
          badge.textContent = 'Active & Agreed';
          badge.className = 'badge badge-emerald';
        } else {
          badge.textContent = 'Consent Pending';
    ...
    acceptTerms() {
      const consent = {
        ...
        version: '1.0.0',
        permissions: ['INTERNET', 'NETWORK_STATE', 'FILE_MANAGER_STORAGE', 'WEARABLE_TELEMETRY']
      };
      localStorage.setItem(TERMS_CONSENT_KEY, JSON.stringify(consent));
      try {
        sessionStorage.setItem('dhc_terms_accepted_session', 'true');
      } catch (e) {}
    ...
    clearTermsValidationError() {
      ...
      if (container) {
        container.style.borderColor = 'rgba(56, 189, 248, 0.25)';
    ...
    showTermsValidationError() {
      ...
      if (container) {
        ...
        container.style.background = 'rgba(239, 68, 68, 0.14)';
    ...
    DOWNLOAD_BACKUP_FILENAME: 'DailyHealthCoachbackupdatabase.json',
    ...
    // Reads Download/DailyHealthCoachbackupdatabase.json or database_backup.json on install/reinstall
    // -------------------------------------------------------------
    ...
    async syncFromDatabaseFileOnStartup() {
      ...
      try {
        ...
        const lastLoadedHash = localStorage.getItem('dhc_loaded_db_file_hash');
        const hasExistingData = localStorage.getItem('dhc_targets_v1') || localStorage.getItem('dhc_habits_v1');
        ...
        if (!hasExistingData || (lastLoadedHash && currentFileHash !== lastLoadedHash)) {
          console.log(`[DailyHealthCoach] Auto-syncing database from ${sourceDir}...`);
          const payload = JSON.parse(rawData);
          await this.applyImportedPayload(payload, sourceDir, false);
          localStorage.setItem('dhc_loaded_db_file_hash', currentFileHash);
          ...
          if (window.toastManager) {
            window.toastManager.show(`✅ Health database synced from ${sourceDir}`, 'success');
          }
        }
      } catch (err) {
    ...
    async exportToFileManager() {
      try {
        ...
        if (Filesystem) {
          try {
            // A. Save in public Download folder: Download/DailyHealthCoachbackupdatabase.json (Survives uninstall)
            try {
            ...
            // B. Write to dedicated internal path Android/data/com.dailyhealthcoach.app/files/database_backup.json
            await Filesystem.writeFile({
              path: fileName,
              data: jsonStr,
              directory: 'EXTERNAL',
            ...
            localStorage.setItem('dhc_loaded_db_file_hash', hash);
          } catch (capErr) {
        ...
        if (!savedPathDescription) {
          ...
          a.href = url;
          a.download = downloadName; // 'DailyHealthCoachbackupdatabase.json'
        ...
        if (window.toastManager) {
          window.toastManager.show(`✅ Database saved to Download folder: ${downloadName}`, 'success');
        }
      } catch (err) {
    ...
    async autoRestoreFromDownloadFolder() {
      ...
      if (window.toastManager) {
        window.toastManager.show('Please select DailyHealthCoachbackupdatabase.json from your Downloads folder', 'info');
      }
    ...
    async applyImportedPayload(payload, sourcePathName = 'File', shouldReload = true) {
      ...
      if (alertEl) {
        ...
        alertEl.style.background = 'rgba(14, 165, 233, 0.2)';
        alertEl.style.borderColor = '#0284c7';
        alertEl.style.color = '#7dd3fc';
        alertEl.textContent = `✅ Health database restored from ${sourcePathName}! (${recordCount} records loaded)`;
      }
    ...
    async importFromFileManager(file) {
      ...
      if (alertEl) {
        alertEl.style.display = 'block';
        alertEl.style.background = 'rgba(14, 165, 233, 0.15)';
        alertEl.style.borderColor = '#0284c7';
        alertEl.style.color = '#7dd3fc';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btnReviewTermsFromProfile')?.addEventListener('click', (e) => {
      e.preventDefault();
